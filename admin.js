...
const LS_USERS = "zf_users";
const ONLINE_TTL = 5 * 60 * 1000; // 5 хв
...
function getOnlineStatus(user) {
  if (!user.lastActive) return "офлайн";
  const t = new Date(user.lastActive).getTime();
  if (Number.isNaN(t)) return "офлайн";
  const diff = Date.now() - t;
  return diff < ONLINE_TTL ? "онлайн" : "офлайн";
}
...
function renderUsersTableAdmin() {
  ...
  if (!USERS.length) {
    tbody.innerHTML =
      '<tr><td colspan="6" style="color:#9ca3af;font-size:13px;">Поки немає зареєстрованих користувачів.</td></tr>';
    return;
  }

  tbody.innerHTML = USERS.map((u, idx) => {
    const status = getOnlineStatus(u);
    const cls =
      status === "онлайн" ? "status-pill status-online" : "status-pill status-offline";
