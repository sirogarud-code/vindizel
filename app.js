...
// ===== USERS / ONLINE STATUS (demo) =====
const ONLINE_TTL = 5 * 60 * 1000; // 5 хвилин
...
function saveUsersLS(list) {
  localStorage.setItem(LS_USERS, JSON.stringify(list));
}
// оновлюємо lastActive в масиві користувачів
(function updateLastActiveOnLogin() {
  const users = loadUsersLS();
  const idx = users.findIndex(u => u.email === user.email); // або u.id === user.id
...
// ===== heartbeat поточного користувача (demo-online) =====
function touchCurrentUser() {
  try {
    const raw = localStorage.getItem(LS_USER);
    if (!raw) return;
    const curr = JSON.parse(raw);
    if (!curr || !curr.email) return;
...
window.addEventListener("load", () => {
  // одразу відмічаємо, що юзер активний
  touchCurrentUser();
  // і далі кожну хв
  setInterval(touchCurrentUser, 60 * 1000);
});
